...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor,
  ...
  if FLAGS.kmeans > 0:
    with tf.name_scope('kmeans') as scope:
      initial_value = tf.truncated_normal(
          [bottleneck_tensor_size, FLAGS.kmeans], stddev=0.001)
      kmeans_weights = tf.Variable(initial_value, name='kmeans_weights')
      ...
      with tf.name_scope('kmeans_losses'):
        ...
        cluster_stats_loss(k_clusters, step_reached_multiplier)
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      ...
      default=1.0,
      help="""\
      The loss weight for kmeans.\
      """
  )
  parser.add_argument(
      '--kmeans_regularization',
      type=float,
      default=0,
      help="""\
      The regularization weight for kmeans.\
      """
  )
  parser.add_argument(
      '--stats_regularization_weight',
      type=float,
      default=0,
      help="""\
      The loss weight for stats regularization.\
      """
  )
  parser.add_argument(
      '--kmeans_loss_multiplier',
      type=float,
      default=1.0,
      help="""\
      The loss weight multiplier for kmeans.\
      """
  )
  parser.add_argument(
      '--kmeans_multiplier_start',
      type=int,
      default=0,
      help="""\
